...
async function recordPromo(localeKey) {
    ...
    // --- Audio Merging (STABLE LEVELS) ---
    console.log(`Merging audio for ${localeKey}...`);
    // Safe Gaps: 2.5, 7.5, 13.0, 19.5, 25.0 (Branding at 25s)
    const offsets = [2.5, 7.5, 13.0, 19.5, 25.0];
    
    // Improved Audio Chain: BG at 0.35, VOs at 1.0. 
    // We mix VOs together first to avoid per-input scaling issues.
    let filterComplex = `[1:a]volume=0.35[bg];`; 
    let voMixInputStr = '';
    ...
    // amix normally divides by N, so we multiply by N (5 VOs currently) to maintain gain.
    filterComplex += `${voMixInputStr}amix=inputs=${localeData.script.length}:normalize=0:dropout_transition=0,volume=${localeData.script.length}[allvo];`;
    // Finally mix BG and allvo
    filterComplex += `[bg][allvo]amix=inputs=2:normalize=0:duration=first[final_audio]`;
